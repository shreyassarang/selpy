...
# ------------------------- GUI Main Window -------------------------
def main_window():
    global main_win, bom_label, db_label, progress_label  

    main_win = tk.Tk()
    main_win.title("MASTER BOM GENERATOR")
    main_win.geometry("450x400")
    main_win.configure(bg="white")

    tk.Label(main_win, text="MASTER BOM GENERATOR", font=("Arial", 14, "bold"), bg="white").pack(pady=10)

    # BOM File Selection
    tk.Label(main_win, text="Select the Input BOM", bg="white", font=("Arial", 10, "bold")).pack()
    bom_label = tk.Label(main_win, text="No file selected", bg="white", fg="red")
    bom_label.pack(pady=5, padx=20, fill="x")
    tk.Button(main_win, text="Select BOM", command=select_bom, width=30).pack(pady=5)

    progress_label = tk.Label(main_win, text="", bg="white", font=("Arial", 10))
    progress_label.pack(pady=5, padx=20, fill="x")

    tk.Button(main_win, text="Process BOM", command=process_bom, bg="green", fg="white", width=30).pack(pady=10)

    # Right-Click Menu
    context_menu = tk.Menu(main_win, tearoff=0)
    context_menu.add_command(label="DataBase", command=database_window)
    context_menu.add_command(label="Passwd_Rst", command=password_reset)
    main_win.bind("<Button-3>", lambda event: context_menu.post(event.x_root, event.y_root))
...
def password_reset():
    ...
    def update_password():
        ...
        PASSWORD = new_pass
        config["password"] = encryptor(PASSWORD)
        save_config(config)  # Save to config.json
        messagebox.showinfo("Success", "Password updated successfully!")
        passwd_window.destroy()


    passwd_window = tk.Toplevel(main_win)
